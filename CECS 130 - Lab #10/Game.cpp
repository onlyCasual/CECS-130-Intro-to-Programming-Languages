...
void Game::SetPlayerTeam(Team team)
{
	this->team = team;
};

void Game::SetTurn(Team turn)
{
	this->turn = turn;
};
...
void Game::ChooseBestMove()
{
		...
		while (cur_in_col != NULL)
		{
			...
			if (i == 0 || i == (this->size - 1))
			{
				...
				while (cur_in_row != NULL)
				{
					...
					if (cur_in_row->team == NONE)
						last_free_rand = cur_in_row;
				...
				if (cur_in_col == &this->board[0][0] || cur_in_col == &this->board[0][this->size - 1])
				{
					...
					while (cur_in_diag != NULL)
					{
						...
						if (cur_in_diag->team == NONE)
							last_free_rand = cur_in_diag;
			...
			if (cur_in_col->team == NONE)
				last_free_rand = cur_in_col;
	...
	else if (last_free_rand != NULL)
		last_free_rand->team = this->turn;
